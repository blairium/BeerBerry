                ...
                if num != None:
                    if num.isnumeric() :
                        ...
                        if harmonic > 0 or harmonic <= 5:
                            ...
                            spec_hm_ienv = maths.get_ienv(i,int(parameters['freq_pert']),harmonic,int(parameters['bandwith_window']), float(parameters['sample_rate']),int(parameters['lpf_bw']),t)
                            int_ienv = maths.cumulative_sum_ienv(spec_hm_ienv)
                            ienv_filtered = maths.filter_ienv(spec_hm_ienv,200)
                            if fig_canvas_agg:
                                destroy_figure(fig_canvas_agg, toolbar)
                            ...
                            harm_one = maths.get_ienv(i,int(parameters['freq_pert']),1,int(parameters['bandwith_window']),
                                            float(parameters['sample_rate']),int(parameters['lpf_bw']),t)
                            harm_two = maths.get_ienv(i,int(parameters['freq_pert']),2,int(parameters['bandwith_window']),
                                            float(parameters['sample_rate']),int(parameters['lpf_bw']),t)
                            harm_three = maths.get_ienv(i,int(parameters['freq_pert']),3,int(parameters['bandwith_window']),
                                            float(parameters['sample_rate']),int(parameters['lpf_bw']),t)
                            harm_four = maths.get_ienv(i,int(parameters['freq_pert']),4,int(parameters['bandwith_window']),
                                            float(parameters['sample_rate']),int(parameters['lpf_bw']),t)
                            harm_five = maths.get_ienv(i,int(parameters['freq_pert']),5,int(parameters['bandwith_window']),
                                            float(parameters['sample_rate']),int(parameters['lpf_bw']),t)
